...
export function readLayoutFromFile(): Record<string, unknown> | null {
	...
	try {
		...
		console.error('[AgentCraft] Failed to read layout file:', err);
		return null;
	}
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
	...
	try {
		...
		console.error('[AgentCraft] Failed to write layout file:', err);
	}
}
...
export function migrateAndLoadLayout(
	...
	if (fromFile) {
		console.log('[AgentCraft] Layout loaded from file');
		return fromFile;
	}
	...
	if (fromState) {
		console.log('[AgentCraft] Migrating layout from workspace state to file');
	...
	if (defaultLayout) {
		console.log('[AgentCraft] Writing bundled default layout to file');
...
/**
 * Watch ~/.agent-craft/layout.json for external changes (other VS Code windows).
...
export function watchLayoutFile(
	...
	function checkForChange(): void {
		...
		try {
			...
			console.log('[AgentCraft] External layout change detected');
			onExternalChange(layout);
		} catch (err) {
			console.error('[AgentCraft] Error checking layout file:', err);
		}
	}
