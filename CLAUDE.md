# AgentCraft — Compressed Reference
...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  AgentCraftViewProvider.ts   — WebviewViewProvider, message dispatch, asset loading
  ...
  layoutPersistence.ts        — User-level layout file I/O (~/.agent-craft/layout.json), migration, cross-window watching
  ...
  performanceScorer.ts        — Performance scoring: calculateScore(), storeScore(), getStoredScores()
  types.ts                    — Shared interfaces (AgentState, PersistedAgent, NotificationPrefs, AgentTemplate, AgentMetrics, FileAccessEvent, PerformanceScore)
...
**Sound notifications**: Ascending two-note chime (E5 → E6) via Web Audio API plays when waiting bubble appears (`agentStatus: 'waiting'`). `notificationSound.ts` manages AudioContext lifecycle; `unlockAudio()` called on canvas mousedown to ensure context is resumed (webviews start suspended). Toggled via "Sound Notifications" checkbox in Settings modal. Enabled by default; persisted in extension `globalState` key `agent-craft.soundEnabled`, sent to webview as `settingsLoaded` on init.

**Smart Notifications** (premium): OS-level alerts via `vscode.window.showWarningMessage`/`showInformationMessage`. Three types, each independently togglable in Settings modal: (1) **Permission Alerts** — fires when permission timer detects a stuck tool, with "Focus Terminal" action button. (2) **Long Task Complete** — fires on `turn_duration` if elapsed time since first tool > `LONG_TASK_THRESHOLD_MS` (2 min default). Tracks `turnStartTime` per agent, `longTaskNotified` prevents duplicates. (3) **Loop Detection** — tracks `recentToolNames` per agent; alerts when same tool appears `LOOP_DETECTION_COUNT` (5) times consecutively. `loopNotified` prevents duplicate alerts per streak. All state resets on new user prompt via `resetTurnNotificationState()`. Preferences stored in `globalState` key `agent-craft.notificationPrefs` as `NotificationPrefs` object. `notificationManager.ts` handles all notification logic. `NotificationEvent` type flows through: `processTranscriptLine` → `readNewLines` → `startFileWatching` → `AgentCraftViewProvider.handleNotification`. Settings UI toggles are greyed out for free users.

**File Heatmap** (premium, F07): `FileHeatmap.tsx` panel toggled via "Files" button. Tracks file access from Read/Edit/Write/Glob/Grep tools via `trackToolMetrics()` in `transcriptParser.ts`. Stores `FileAccessEvent[]` per agent (capped at `MAX_FILE_EVENTS_PER_AGENT` = 500). Sends `agentFileAccess` messages to webview in real-time. Panel shows files sorted by touch frequency, color-coded (green=read-only, yellow=edited once, red=many edits), conflict indicator ("!" when 2+ agents edit same file), per-agent filter buttons. Click opens file via `openFile` message → `vscode.window.showTextDocument()`. `AgentMetrics` on `AgentState` accumulates `toolCounts`, `filesTouched` (Set), `filesEdited` (array), `permissionWaitCount`, `turnCount`, `loopCount`.

**Agent Orchestration** (premium, F09): `OrchestrationPanel.tsx` toggled via "Team" button. Two tabs: **Standup** — all agents listed with status dot, current/last activity one-liner, sub-agent count badge; click focuses terminal. **Sub-agents** — tree view grouped by parent agent, shows sub-agent IDs + task labels. Uses existing `subagentCharacters` + `agentTools` state. `agentLastActivity` state tracks latest tool status text per agent.

**Performance Scoring** (premium, F10): `performanceScorer.ts` calculates score 0-100 on agent session close. Formula: `100 - loopPenalty(loopCount*10, cap 40) - revertPenalty(consecutive same-file edits*15, cap 30) - idlePenalty(permissionWaits*2, cap 20)`. Scores stored in `globalState` key `agent-craft.performanceScores` (up to 20). `PerformanceCard.tsx` panel toggled via "Score" button — shows color-coded score bar (green≥80, yellow≥60, orange≥40, red<40), breakdown, tool/turn/file counts. Active agents show "Running..." until terminal closes.
...
**Layout model**: `{ version: 1, cols, rows, tiles: TileType[], furniture: PlacedFurniture[], tileColors?: FloorColor[] }`. Grid dimensions are dynamic (not fixed constants). Persisted via debounced saveLayout message → `writeLayoutToFile()` → `~/.agent-craft/layout.json`.
